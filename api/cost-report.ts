...
function isValidPayload(body: unknown): body is ReportRequest {
  ...
  if (b.firstName !== undefined && (typeof b.firstName !== "string" || b.firstName.length > 40)) return false;
...
function sanitizeFirstName(raw: string | undefined): string | null {
  if (!raw) return null;
  const trimmed = raw.trim().slice(0, 40);
  // Strip control characters; keep letters, spaces, hyphens, apostrophes,
  // dots — covers typical names (incl. accented letters via \p{L}).
...
async function subscribeAndTagBeehiiv(email: string, country: string, firstName: string | null): Promise<void> {
  ...
  if (firstName) {
    subscribeBody.custom_fields = [{ name: "First Name", value: firstName }];
  }
...
function buildContent(doc: PDFKit.PDFDocument, req: ReportRequest, totals: ComputedTotals): number {
  ...
  const titleText = cleanName
    ? `For ${cleanName}, your ESL cost estimate — ${req.country.name}, ${weeksLabel}`
    : `Your ESL Cost Estimate — ${req.country.name}, ${weeksLabel}`;
  ...
  doc.font("Helvetica").fontSize(9).fillColor(MUTED)
    .text(`Generated by ESLVolunteerFinder.com  ·  ${generatedDate}`, SIDE_MARGIN, y, { lineBreak: false });
  y += 16;
...
export default async function handler(req: any, res: any) {
  ...
  try {
    await subscribeAndTagBeehiiv(body.email, body.country.name, sanitizeFirstName(body.firstName));
